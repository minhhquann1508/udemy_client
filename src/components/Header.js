...
import { Categories, SearchBox, Button, NavBar } from './'
...
function Header() {
    ...
    return (
        <>
            <NavBar isActive={showNav} setIsActive={setShowNav} />
            <header className='w-full shadow-md'>
                <div className='px-8 py-2.5 h-full flex justify-between md:justify-normal items-center text-[13px] text-main-black gap-4'>
                    ...
                    <button onClick={() => navigate(`/my-info/${user?.id}`)}
                        className={`${checkLogin() ? 'hidden md:block' : 'hidden'} border py-2 h-full border-[#000] px-3 hover:scale-105 duration-200`}>
