...
export const renderStar = (number) => {
    ...
    if (isIntergerNum) {
        for (let i = 0; i < number; i++) {
            html.push(<span key={i}><icons.IoStar color='#f69c08' /></span>)
        }
        for (let i = 5; i > number; i--) {
            html.push(<span key={i}><icons.IoStarOutline color='#f69c08' /></span>)
        }
    } else {
        const intNum = Math.floor(number);
        for (let j = 0; j < intNum; j++) {
            html.push(<span key={j}><icons.IoStar color='#f69c08' /></span>)
        }
